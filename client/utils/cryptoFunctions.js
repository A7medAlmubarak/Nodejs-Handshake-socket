...
async function generateCSR(privateKeyArmored, userId) {
  try {
    const csrData = `User: ${userId}`;
    const { keys: [privateKey] } = await openpgp.key.readArmored(privateKeyArmored);

    const { signature } = await openpgp.sign({
      message: openpgp.cleartext.fromText(csrData),
      privateKeys: [privateKey],
    });

    return { userId, csrData, signature: signature || '' };
  } catch (error) {
...
async function verifySignature(data, signature, publicKeyArmored) {
